...
def get_Gravity_Matrix(x, g=9.81, alpha=0, additional_mass=0):
    ...
    G = np.array(
        [
            g
            * (
                m1 * lc1 * np.cos(x[0] + alpha)
                + m2 * (lc2 * np.cos(x[0] + alpha + x[3]) + l1 * np.cos(x[0] + alpha))
                + additional_mass
                * (l2 * np.cos(x[0] + alpha + x[3]) + l1 * np.cos(x[0] + alpha))
            ),
    ...
    Gdot = np.array(
        [
            g
            * (
                -m1 * lc1 * np.sin(x[0] + alpha) * x[1]
                + m2
                * (
                    -lc2 * np.sin(x[0] + alpha + x[3]) * (x[1] + x[4])
                    - l1 * np.sin(x[0] + alpha) * x[1]
                )
                + additional_mass
                * (
                    -l2 * np.sin(x[0] + alpha + x[3]) * (x[1] + x[4])
                    - l1 * np.sin(x[0] + alpha) * x[1]
                )
            ),
            -g
            * (m2 + l2 / lc2 * additional_mass)
            * lc2
            * np.sin(x[0] + alpha + x[3])
            * (x[1] + x[4]),
        ]
    )
    return G, Gdot
    ...
    return np.array(
        [
            [
                ...
                + I1
                + I2
                + m2 * (lc2 * lc2 + l1 * l1 + 2 * l1 * lc2 * cos(te))
                + ObjectMass * (l1 * l1 + 2 * l1 * l2 * cos(te) + l2 * l2),
                I2
                + m2 * (lc2 * lc2 + l1 * lc2 * cos(te))
                + ObjectMass * (l1 * l2 * cos(te) + l2 * l2),
            ],
            [
                I2
                + m2 * (lc2 * lc2 + l1 * lc2 * cos(te))
...
def dM1(te, oe, ObjectMass=0, Ms=52, Ls=1.57):
    ...
    return np.array(
        [
            [
                m2 * (-2 * l1 * lc2 * sin(te) * oe)
                + ObjectMass * (-2 * l1 * l2 * sin(te) * oe),
                m2 * (-l1 * lc2 * sin(te) * oe)
                + ObjectMass * (-l1 * l2 * sin(te) * oe),
            ],
            [
                m2 * (-l1 * lc2 * sin(te) * oe)
                + ObjectMass * (-l1 * l2 * sin(te) * oe),
...
def dC1(te, os, oe, accs, acce, ObjectMass=0, Ms=52, Ls=1.57):
    ...
    C = np.array([[-2 * oe * c, -oe * c], [os * c, 0]])
    Cdot = np.array(
        [[-2 * (oe * dc + acce * c), -(oe * dc + acce * c)], [(os * dc + accs * c), 0]]
    )
