...
def step5(
    ...
    Omega_measure = np.diag(np.ones(6)) * 1e-4
    F = 0
    for i in range(Num_steps):
        tv = torque_val if i*dt >= torque_time else [0, 0]
        F = (
        ...
        newx[i + 1, :Num_Var] = newx[i, :Num_Var] + dt * f(newx[i, :Num_Var], u, F, tv)
